...
public class GeoRecommendation {
	...
	public List<Item> recommendItems(String userId, double lat, double lon) {
		...
		for (String favItemId : favItemIds) {
			Set<String> categories = conn.getCategories(favItemId);
		...
		for (Entry<String, Integer> entry : categoryList) {
			...
			for (Item item : items) {
				if (!visitedItemIds.contains(item.getItemId()) && !favItemIds.contains(item.getItemId())) {
